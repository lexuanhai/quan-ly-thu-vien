...
namespace PhanMemQLTV
{
    public partial class frmBaoCaoThongKe : Form
    {
        ...
        {
            string strTinhSLSachQuaHan = "select Count(distinct(ctpm.MaSach)) as TongSLQuaHan " +
              " from tblHSPhieuMuon phieumuon " +
              " inner join ChiTietPM ctpm on ctpm.MaPhieuMuon = phieumuon.MaPhieu " +
              " inner join tblDocGia docgia on phieumuon.MaDG = docgia.MaDG where ctpm.TinhTrang = N'Đang Mượn' and CONVERT (datetime, ctpm.NgayTra, 103) < getdate()";
        ...
        private void slDGQuaHan()
        {
            ...
            string strTinhSLDGQuaHan = "select count(distinct(phieumuon.MaDG)) as TongSLQuaHan " +
              " from tblHSPhieuMuon phieumuon " +
              " inner join ChiTietPM ctpm on ctpm.MaPhieuMuon = phieumuon.MaPhieu " +
              " inner join tblDocGia docgia on phieumuon.MaDG = docgia.MaDG where phieumuon.TinhTrang = N'Đang Mượn' and CONVERT (datetime, ctpm.NgayTra, 103) < getdate()";
            myConnection = new SqlConnection(strKetNoi);
            myConnection.Open();
        ...
        private void btnXemSLSachQuaHan_Click(object sender, EventArgs e)
        {
            ...
            string query = "select ctpm.MaPhieuMuon as N'Mã Phiếu', phieumuon.MaDG as N'Mã DG',ctpm.MaSach as 'Mã Sách', ctpm.NgayMuon as N'Ngày Mượn', ctpm.NgayTra as N'Ngày Trả',ctpm.SoLuong as N'Số Lượng' from tblHSPhieuMuon phieumuon " +
               "inner join ChiTietPM ctpm on ctpm.MaPhieuMuon = phieumuon.MaPhieu " +
               "inner join tblDocGia docgia on phieumuon.MaDG = docgia.MaDG where ctpm.TinhTrang = N'Đang Mượn'";
           //string strTimSLSQuaHan = @"SELECT MaPhieuMuon as 'Mã Phiếu', MaDG as 'Mã DG', MaSach as 'Mã Sách', NgayMuon as 'Ngày Mượn', NgayTra as 'Ngày Trả', GhiChu as 'Ghi Chú' from ChiTietPM where CONVERT (datetime, NgayTra, 103) < getdate()";
        ...
        private void btnBaocao_Click(object sender, EventArgs e)
        {
            ...
            string query = "select theloai.TenTheLoai as N'Thể loại',SUM(ctpm.SoLuong) as N'Số Lần Mượn' from tblSach sach  inner join tblTheLoai theloai on theloai.MaTheLoai = sach.MaTheLoai inner join ChiTietPM ctpm on ctpm.MaSach = sach.MaSach where CONVERT (datetime, ctpm.NgayTra, 103) < getdate() group by theloai.TenTheLoai";
            dataGridViewDSDGQuaHan.DataSource = ketnoi(query);
            dataGridViewDSDGQuaHan.Columns["Số Lần Mượn"].Width = 440;
            dataGridViewDSDGQuaHan.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            dataGridViewDSDGQuaHan.Show();
        ...
        private void btnSLDGQuaHan_Click(object sender, EventArgs e)
        {
            ...
            string strTimSLDGQuaHan = "select phieumuon.MaDG as N'Mã DG', sum(phieumuon.SLMuon) as N'SL Sách Mượn'   from tblHSPhieuMuon phieumuon " +
                " inner join ChiTietPM ctpm on ctpm.MaPhieuMuon = phieumuon.MaPhieu " +
                "inner join tblDocGia docgia on phieumuon.MaDG = docgia.MaDG where phieumuon.TinhTrang = N'Đang Mượn' and CONVERT (datetime, ctpm.NgayTra, 103) < getdate() group by phieumuon.MaDG";
